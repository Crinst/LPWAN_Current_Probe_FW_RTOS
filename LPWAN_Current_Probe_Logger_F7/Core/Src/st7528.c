...
uint8_t controlB = ST7528_COMMAND_TX;
const char Slave = 0x3f;
// Foreground color
...
static void ST7528_cmd(uint8_t cmd) {
	...
	buffi2c[0] = controlB;
	buffi2c[1] = cmd;
	HAL_I2C_Master_Transmit(&hi2c1, Slave << 1, buffi2c,sizeof(buffi2c), 200);
...
void ST7528_Init(void) {
	...
	HAL_GPIO_WritePin(LCD_RST_GPIO_Port,LCD_RST_Pin, GPIO_PIN_SET); //reset pin LCD
	...
	HAL_Delay(1); // The maximum reset duration (tR) is 1us at VDD=3.3V and 2us at VDD=1.8V

	// Initial display configuration

	// ICON disable
	ST7528_cmd(ST7528_CMD_ICON_OFF);	//(uint8_t)0xA2

	// Display OFF
	ST7528_cmd(ST7528_CMD_DISPOFF);		//(uint8_t)0xAE

	// Set duty cycle: 128
	ST7528_cmd_double(ST7528_CMD_PD_DUTY,128);		//(uint8_t)0x48

	// Set initial COM0: 0
	ST7528_cmd_double(ST7528_CMD_COM0,0);		//(uint8_t)0x44

	// Set initial display line: 0
	ST7528_cmd_double(ST7528_CMD_LINE,0);		//(uint8_t)0x40

	// Set COM scan direction (SHL bit): normal
	ST7528_cmd(ST7528_CMD_SHL_OFF);			//(uint8_t)0xC0

	// Set SEG scan direction (ADC bit): normal
	ST7528_cmd(ST7528_CMD_ADC_OFF);			//(uint8_t)0xA0

	// Enable built-in oscillator circuit
	ST7528_cmd(ST7528_CMD_OSCON);			//(uint8_t)0xAB

	// Set internal resistance ratio of the regulator resistor
	ST7528_cmd(ST7528_CMD_RREG | ST7528_RREG_72);		//(uint8_t)0x20		(uint8_t)0x07  // 7.2

	// Set LCD bias
	ST7528_cmd(ST7528_CMD_BIAS | ST7528_BIAS_11);		//(uint8_t)0x50		(uint8_t)0x06, // 1/11

	// Electronic volume
	ST7528_cmd_double(ST7528_CMD_ELVOL,32);			//(uint8_t)0x81

	// Configure FRC/PWM mode
	ST7528_cmd(ST7528_CMD_FRC_PWM | ST7528_FRC_4 | ST7528_PWM_60);		//(uint8_t)0x90		(uint8_t)0x00  // 4FRC		(uint8_t)0x02  // 60PWM

	// DC-DC DC[1:0]=00 booster 3X
	ST7528_cmd(ST7528_CMD_DCDC | ST7528_BOOST_3X);			//(uint8_t)0x64		(uint8_t)0x00, // 3 times boosting
	// Delay 200ms
	HAL_Delay(200);

	// Power control (VC=1,VR=0,VF=0): turn on internal voltage converter circuit
	ST7528_cmd(ST7528_CMD_PWR | ST7528_PWR_VC);			//(uint8_t)0x28		(uint8_t)0x04 // Internal voltage converter circuit ON

	// DC-DC DC[1:0]=11 booster 6X
	ST7528_cmd(ST7528_CMD_DCDC | ST7528_BOOST_6X);			//(uint8_t)0x64		(uint8_t)0x03  // 6 times boosting
	// Delay 200ms
	HAL_Delay(200);

	// Power control (VC=1,VR=1,VF=0): turn on internal voltage regulator circuit
	ST7528_cmd(ST7528_CMD_PWR | ST7528_PWR_VC | ST7528_PWR_VR);			//(uint8_t)0x28		(uint8_t)0x04		(uint8_t)0x02
	// Delay 10ms
	HAL_Delay(10);

	// Power control (VC=1,VR=1,VF=1): turn on internal voltage follower circuit
	ST7528_cmd(ST7528_CMD_PWR | ST7528_PWR_VC | ST7528_PWR_VR | ST7528_PWR_VF);			//(uint8_t)0x28		(uint8_t)0x04		(uint8_t)0x02		(uint8_t)0x01

	// Set EXT=1 mode
	ST7528_cmd_double(ST7528_CMD_MODE,ST7528_MODE_EXT1);		//(uint8_t)0x38		(uint8_t)0x01

	// Set white mode and 1st frame
	ST7528_cmd_double(ST7528_CMD1_W1F,0x00);		//(uint8_t)0x80
	// Set white mode and 2nd frame
	ST7528_cmd_double(ST7528_CMD1_W2F,0x00);		//(uint8_t)0x81
	// Set white mode and 3rd frame
	ST7528_cmd_double(ST7528_CMD1_W3F,0x00);		//(uint8_t)0x82
	// Set white mode and 4th frame (not used in 3FRC mode)
	ST7528_cmd_double(ST7528_CMD1_W4F,0x00);		//(uint8_t)0x83

	// Load grayscale palette
	for (i = 0; i < sizeof(GrayPalette); i++) {
		ST7528_cmd_double(ST7528_CMD1_GRAYPAL + i,GrayPalette[i]);		//(uint8_t)0x84 1 ... 14
	}

	// Set dark mode and 1st frame
	ST7528_cmd_double(ST7528_CMD1_D1F,0x3c);			//(uint8_t)0xBC
	// Set dark mode and 2nd frame
	ST7528_cmd_double(ST7528_CMD1_D2F,0x3c);			//(uint8_t)0xBD
	// Set dark mode and 3rd frame
	ST7528_cmd_double(ST7528_CMD1_D3F,0x3c);			//(uint8_t)0xBE
	// Set dark mode and 4th frame (not used in 3FRC mode)
	ST7528_cmd_double(ST7528_CMD1_D4F,0x3c);			//(uint8_t)0xBF

	// Set EXT=0 mode, booster efficiency level and frame frequency
	ST7528_cmd_double(ST7528_CMD_MODE,ST7528_MODE_EXT0 | ST7528_MODE_BE1 | ST7528_FF_77);		//(uint8_t)0x38		(uint8_t)0x00		(uint8_t)0x00		(uint8_t)0x00, // 77Hz (+/- 5%)

	// Configure N-line inversion to reduce display pixels crosstalk
	// Recommended to used the odd number from range of 3..33
	//ST7528_cmd_double(ST7528_CMD_NLINE_INV,15);			//(uint8_t)0x4C

	// Display ON
	ST7528_cmd(ST7528_CMD_DISPON);				//(uint8_t)0xAF
}
...
static void ST7528_send_buff(uint8_t *pBuf, uint32_t count) {

	uint8_t buf[count + 1];
	buf[0] = ST7528_DATA_TX;
	int i = 0;
	...
	HAL_I2C_Master_Transmit(&hi2c1, Slave << 1, (uint8_t *) buf, sizeof(buf),
			200);
...
void ST7528_Flush(void) {
	...
	buf[0] = ST7528_COMMAND_TX;
	// Column MSB always zero
	buf[1] = ST7528_CMD_COLL;
	buf[2] = ST7528_CMD_COLM;
	// Start from page 0
	buf[3] = ST7528_CMD_PAGE;

	// Send vRAM to display by pages
	while (buf[3] < ST7528_CMD_PAGE + 13) {
		// Write page/column address
		HAL_I2C_Master_Transmit(&hi2c1, Slave << 1, buf,sizeof(buf), 200);
		// Transmit one page
		ST7528_send_buff(ptr, SCR_PAGE_WIDTH*4);
...
void ST7528_Contrast(uint8_t res_ratio, uint8_t lcd_bias, uint8_t el_vol) {
	...
	buf[0] = controlB;
	buf[1] = ST7528_CMD_RREG | (res_ratio & 0x07);
	// Select LCD bias
	buf[2] = ST7528_CMD_BIAS | (lcd_bias & 0x07);
	// Select electronic volume (double byte command)
	buf[3] = ST7528_CMD_ELVOL;
	buf[4] = el_vol & 0x3f;
...
void LCD_Pixel(uint8_t X, uint8_t Y, uint8_t GS) {
	...
	if (scr_orientation & (SCR_ORIENT_CW | SCR_ORIENT_CCW)) {
		...
		ptr = (uint32_t *) &vRAM[(((X >> 3) << 9) + ((X >> 3) << 7)) + (Y << 2)];
	} else {
		voffs = Y & 0x07;
		ptr = (uint32_t *) &vRAM[(((Y >> 3) << 9) + ((Y >> 3) << 7)) + (X << 2)];
	}
...
static void LCD_HLineInt(uint8_t X, uint8_t Y, uint8_t W, uint8_t GS) {
	...
	register uint32_t *ptr = (uint32_t *) &vRAM[(((Y >> 3) << 9) + ((Y >> 3) << 7)) + (X << 2)];;
